...
if(isset($_POST['submit']) && isset($_POST['insertblog'])){
	...
	$date = date('Y-m-d');
  	$sql = "INSERT INTO blog (blogtitle,blogdiscription,blogimg,create_date
) VALUES ('$blogtitle','$blogdiscription','$target','$date')";
  	$conn->query($sql);
...
if(isset($_POST['submit']) && isset($_POST['editblog'])){
	...
	if($blogimg_temp != ""){
	    ...
	    $updateblog = "UPDATE blog SET blogtitle='$blogtitle',blogdiscription='$blogdiscription', blogimg='$target' WHERE id='$blogid'";   
	}else{
